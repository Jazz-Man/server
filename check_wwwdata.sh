...
if [[ $(find ${NGINX_DOCROOT} ! -perm 755 -type d | wc -l) -gt 0 ]] || [[ $(find ${NGINX_DOCROOT} ! -perm 644 -type f | wc -l) -gt 0 ]]; then
    ...
    /usr/bin/env sh -c 'find {{NGINX_DOCROOT}} -type d -exec chmod 755 {} \; && find {{NGINX_DOCROOT}} -type f -exec chmod 644 {} \;'
 else
...
if [[ $(find ${NGINX_DOCROOT} ! -user www-data | wc -l) -gt 0 ]]; then
    ...
    /usr/bin/env sh -c 'find /usr/share/nginx/html -type d -exec chown www-data:www-data {} \; && find {{NGINX_DOCROOT}} -type f -exec chown www-data:www-data {} \;'
 else
